...
function Usuarios() {
    ...
    const handleSubmitFormulario = async (e) => {
        ...
        try {
            if (editando) {
                // Modo edición: solo actualiza nombre y rol
                await axios.put(`/api/usuarios/${userId}`, { name, role }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                // Modo creación: guarda un nuevo usuario con su clave inicial
                await axios.post('/api/usuarios', { name, email, password: passwordForm, role }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            cancelarAccion();
    ...
    return (
        <div style={{ background: '#fff', padding: '20px', borderRadius: '5px', maxWidth: '950px', margin: '20px auto' }}>
            ...
            {/* FORMULARIO ÚNICO: AGREGAR O EDITAR DATOS (Exclusivo para Admins) */}
            {esAdmin && !cambiandoPass && (
                <form onSubmit={handleSubmitFormulario} style={{ background: '#f9f9f9', padding: '15px', marginBottom: '20px', borderRadius: '4px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'flex-end' }}>
                        ...
                        {/* El email solo se puede escribir al crear, no al editar */}
                        <div>
                        ...
                        {/* La contraseña solo se solicita al crear un registro nuevo */}
                        {!editando && (
                            <div>
                        ...
                        <div>
                            ...
                            <select value={role} onChange={(e) => setRole(e.target.value)} style={{ padding: '5px', width: '150px' }}>
            ...
            {/* FORMULARIO INDEPENDIENTE: CAMBIAR LA CONTRASEÑA */}
                        ...
                        <tr key={user.id} style={{ backgroundColor: user.activo ? '#fff' : '#fce8e6' }}>
                            ...
                            {esAdmin && (
                                <td>
                                    ...
                                    {/* Botón Booleano para activar/desactivar */}
                                    <button 
